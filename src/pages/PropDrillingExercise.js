...
const RootComponent = (props) => {
  ...
  const addProductToCart = ({ id, price }) => {
    setCart((prevCart) => {
      const correspondingCart = prevCart.products.find(
        (cart) => cart.id === id
      );
      correspondingCart.price += price;
      correspondingCart.qty += 1;
  ...
  const removedProduct = ({ id, price }) => {
    setCart((prevCart) => {
      const correspondingCart = prevCart.products.find(
        (cart) => cart.id === id
      );

      if (correspondingCart.qty > 0) {
        correspondingCart.qty -= 1;
        correspondingCart.price -= price;
        cart.totalPrice -= price;
      }
      ...
      <Grid container justifyContent="center">
        ...
        <Grid item xs={8}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <Button
              onClick={() => props.removedProduct(props.product)}
              variant="error"
              sx={{ width: "5rem" }}
            >
              Remove
